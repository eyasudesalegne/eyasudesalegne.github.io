(() => {
  ...
  if (title) title.textContent = 'A life assembled in layers';

  const text = document.getElementById('injeraStoryText');
  if (text) text.textContent = 'Identity is rarely formed all at once. It grows through experience, relationships, culture, discipline, and the values we choose to keep.';

  document.querySelectorAll('.injera-loading,.injera-story-hint').forEach((item) => item.remove());
  ...
  window.addEventListener('resize', () => {
    const imgs = loaded();
    if (imgs[0]) draw(imgs[0], imgs[1] || imgs[0], 0);
  });
  init();
})();
